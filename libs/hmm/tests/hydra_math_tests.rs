#[cfg(test)]
mod tests {
    use spl_math::precise_number::{ONE, PreciseNumber};
    use spl_math::uint::U256;
    use hydra_math::swap_result::SwapResult;
    use hydra_math::math::{sqrt, log, checked_pow_fraction, signed_addition, signed_mul};
    ...
    fn test_precise_number_storage() {
        ...
        let expected = PreciseNumber { value: InnerUint::from(3_141_592_653_589u128) };
    ...
    fn test_u128_natural_log() {
        ...
        let precision = InnerUint::from(ONE);
        let expected = PreciseNumber { value: InnerUint::from(6_907_755_278_982u128) };
        assert!(log(1_000u128).almost_eq(&expected, precision));

        // ln(18446744073709551615) = 44.3614195558365
        let expected = PreciseNumber { value: InnerUint::from(44_361_419_555_836u128) };
    ...
    fn test_checked_pow_fraction() {
        ...
        assert_eq!(checked_pow_fraction(&base, &exp), PreciseNumber::new(1u128).unwrap());

        // 42^0.25 = 2.545729895021831
        let exp = PreciseNumber::new(1u128).unwrap()
            .checked_div(&PreciseNumber::new(4u128).unwrap()).unwrap();
        let expected = PreciseNumber { value: InnerUint::from(2_545_729_895_021u128) };
        assert!(checked_pow_fraction(&base, &exp).almost_eq(&expected, precision));

        // 42^0.5 = 6.48074069840786
        let exp = PreciseNumber::new(1u128).unwrap()
            .checked_div(&PreciseNumber::new(2u128).unwrap()).unwrap();
        let expected = PreciseNumber { value: InnerUint::from(6_480_740_698_407u128) };
        assert!(checked_pow_fraction(&base, &exp).almost_eq(&expected, precision));

        // 42^1 = 42
        let exp = PreciseNumber::new(1u128).unwrap();
        assert_eq!(checked_pow_fraction(&base, &exp), PreciseNumber::new(42u128).unwrap());

        // 42^1.25 = 106.920655590916882
        let exp = PreciseNumber::new(5u128).unwrap()
            .checked_div(&PreciseNumber::new(4u128).unwrap()).unwrap();
        let expected = PreciseNumber { value: InnerUint::from(106_920_655_590_916u128) };
        assert!(checked_pow_fraction(&base, &exp).almost_eq(&expected, precision));

        // 42^1.5 = 272.19110933313013
        let exp = PreciseNumber::new(3u128).unwrap()
            .checked_div(&PreciseNumber::new(2u128).unwrap()).unwrap();
        let expected = PreciseNumber { value: InnerUint::from(272_191_109_333_130u128) };
        assert!(checked_pow_fraction(&base, &exp).almost_eq(&expected, precision));

        // 42^2 = 1764
        let exp = PreciseNumber::new(2u128).unwrap();
        assert_eq!(checked_pow_fraction(&base, &exp), PreciseNumber::new(1764u128).unwrap());
    }
    ...
    fn test_signed_addition() {
        ...
        assert_eq!(signed_addition(&lhs, lhs_signed, &rhs, rhs_signed), (expected, expected_signed));
    ...
    fn test_signed_mul() {
        ...
        let expected_signed = false;
        assert_eq!(signed_mul(&lhs, lhs_signed, &rhs, rhs_signed), (expected, expected_signed));
