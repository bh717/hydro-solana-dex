...
mod tests {
    ...
    2000_f64.sqrt(), 1, 0.0, 0.0) ;
        ...
        assert_eq!(x_adj, 0_f64); assert_eq!(x_fee, 0_f64);
        assert_eq!(y_adj, 0_f64); assert_eq!(y_fee, 0_f64);
    }
    ...
    2000_f64.sqrt(), 1, 0.0, 0.0) ;
        ...
        let mut pool_bis = Pool::new("ETH", 18, "USDC",6,
    2000_f64.sqrt(), 1, 0.0, 0.0) ;
        ...
        // out of liquidity 
        assert_eq!(pool.glbl_liq(),0.0); assert_eq!(pool_bis.glbl_liq(),0.0);
        ...
        assert_eq!(extra.0 ,0.0 ); assert_eq!(extra.1 ,0.0 );
        assert_eq!(extra_bis.0 ,0.0 ); assert_eq!(extra_bis.1 ,0.0 );
    }

    #[test]
    fn test_pool_starts_w_no_liq_in_range_x() {
        let mut pool = Pool::new("SOL", 12, "USDC",6,
    150_f64.sqrt(), 1, 0.0, 0.0) ;
        pool.deposit("dude",4.0, 600.0, 100_f64.sqrt(),140_f64.sqrt());
        
        // no x should be have been deposited in pool, given range. 
        assert_eq!(pool.x_info().0, 0.0); assert!(pool.y_info().0 > 0.0); 
        ...
        assert!(rez.0 > 0.0); assert!(rez.1 < 0.0);
        assert!(pool.glbl_liq() > 0.0); 
        assert!(pool.x_info().0 > 0.0);
        assert!(pool.glbl_rp().powi(2) > 100.0); assert!(pool.glbl_rp().powi(2) < 150.0);
    ...
    150_f64.sqrt(), 1, 0.0, 0.0) ;
        ...
        // no x should be have been deposited in pool, given range. 
        assert!(pool.x_info().0 > 0.0); assert_eq!(pool.y_info().0, 0.0); 
        assert_eq!(pool.glbl_liq(),0.0); // no liq in range
        ...
        assert!(pool.glbl_rp().powi(2) > 180.0); assert!(pool.glbl_rp().powi(2) < 230.0);
