...
mod tests {
    ...
    #[test]
    fn test_hmm_no_fees() {
        let mut amm_pool = Pool::new("ETH", 18, "USDC",6,
    2000_f64.sqrt(), 1, 0.0, 0.0) ;
        ...
        let mut hmm_pool = Pool::new("ETH", 18, "USDC",6,
    2000_f64.sqrt(), 1, 1.5, 0.0) ;
        ...
        assert_eq!(x_a, x_h); assert_eq!(x_adj_a, x_adj_h); assert_eq!(x_fee_a, x_fee_h);
        assert_eq!(y_a, y_h); assert_eq!(y_adj_a, y_adj_h); assert_eq!(y_fee_a, y_fee_h);
    ...
    fn test_fees() {
        ...
        let mut hmm_nofee = Pool::new("ETH", 18, "USDC",6,
2000_f64.sqrt(), 1, 1.5, 0.0) ;
        hmm_nofee.deposit("abc",2.0, 4000.0, 1333_f64.sqrt(),3000_f64.sqrt());
        let x_in_pool_n = hmm_nofee.x_info().0; 
        
        let mut hmm_feed = Pool::new("ETH", 18, "USDC",6,
        2000_f64.sqrt(), 1, 1.5, fee_rate) ;
        ...
        if x_to_swap > x_in_pool_n {
            ...
            assert!(rez_f.5 >  rez_n.5);
            // x_fee accounts for the difference between (x post_swap)  and (amt sent in + x pre_swap) //* here small diff due to error in fee_x 'reverse-engineering' (line 774 )
            assert!(x_in_pool_f + rez_f.0 - (hmm_feed.x_info().0 + rez_f.3) < rez_f.3 * 1e-12); 
            assert_eq!(x_in_pool_n + rez_n.0 , hmm_nofee.x_info().0 + rez_n.3); 
    ...
    fn test_small_trades() {
        let mut pool = Pool::new("HYS", 12, "USDC",6,
        10000_f64.sqrt(), 1, 0.0, 0.003_f64) ;
